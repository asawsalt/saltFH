...
const data_link =
  'https://gist.githubusercontent.com/ksukrit/a2c278cd6f5832f929a421a013ab52db/raw/0f979f47826e224491b4468e8b5f5b1b3d697d01/prompt_advice_filtered.json';
...
export default function App() {
  ...
  const getCombination = useCallback((selectObjects) => {
    selectObjects = selectObjects.map(
      (id) => Users.find((item) => item.id === id).name
    );
    selectObjects.sort();
  ...
  }, [top5Selected, getData, selectedCards]);
  ...
  const renderFinancialHoroscope = () => {
    ...
    let horoscopeMessage = financialHoroscope;
    if (horoscopeMessage) {
      // replace {PERSON_NAME} with name
      horoscopeMessage = horoscopeMessage.replace('{PERSON_NAME}', userName);
    }
      ...
      return (
        <View
          ...
          <Text style={{ fontSize: 24, marginBottom: 10 }}>
            Horoscope not available for selected categories
          </Text>
      ...
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
        <View
          ...
          <Text style={{ marginTop: 10, textAlign: 'center' }}>
            Get started on your financial wellness journey by downloading the
            Salt App
          </Text>
          <Button
            title="Download App"
            onPress={() => Linking.openURL('https://salt.one/')}
          />

          {/* Additional message inside the card borders */}
          <View
            style={{
              position: 'absolute',
              bottom: 10,
      ...
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
            ...
            <TouchableOpacity
              ...
              <Image
                style={{
                  ...
                  borderColor: selectedCards.includes(item.id)
                    ? 'black'
                    : 'transparent',
                }}
                source={item.uri}
              ...
              <Text
                style={{
                  ...
                  marginTop: 5,
                  fontSize: 16,
                  fontWeight: selectedCards.includes(item.id)
                    ? 'bold'
                    : 'normal',
                }}>
                {item.name}
        ...
        {!isContinueButtonEnabled && (
          <Text style={{ fontSize: 16, marginTop: 10 }}>
            Select {5 - selectedCards.length} more
          </Text>
        )}
        {isContinueButtonEnabled && (
          <TouchableOpacity
  ...
  const renderNameInput = () => {
    return (
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
        <Text style={{ fontSize: 24, marginBottom: 10 }}>
          Welcome to the Financial Horoscope App!
        </Text>
        <Text style={{ fontSize: 24, marginBottom: 10 }}>
          What is your name?
        </Text>
    ...
    if (likedCards.length < 5) {
      return (
        <View
          style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
          <Text style={{ fontSize: 24, marginBottom: 10 }}>
            Hi {userName}, you've only selected {likedCards.length}
          </Text>
          <Text style={{ fontSize: 24, marginBottom: 10 }}>
            You need to select atleast 5 cards
          </Text>
          <Button
            title="Restart"
            onPress={() => {
  ...
  return (
    <View style={{ flex: 1 }}>
      <Text>
        {currentIndex + 1} out of {Users.length} and Selected{' '}
        {likedCards.length} so far
      </Text>
      ...
      <View style={{ height: 40 }}></View>
      <View
        style={{ height: 20, width: SCREEN_WIDTH, backgroundColor: 'gray' }}>
        <Animated.View
          style={{
            height: 20,
            width: (SCREEN_WIDTH / Users.length) * (currentIndex + 1),
            backgroundColor: 'green',
          }}
        />
        <Text style={{ textAlign: 'center', fontSize: 16, fontWeight: 'bold' }}>
          {Math.round((currentIndex / Users.length) * 100)}%
        </Text>
      </View>
      ...
      {currentIndex > 0 && (
        <Button
          ...
          onPress={() => {
            if (currentIndex > 0) {
              setCurrentIndex(currentIndex - 1);
              setLikedCards(likedCards.slice(0, -1));
            }
          }}
        />
      )}
